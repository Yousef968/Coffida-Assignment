...
class AddReview extends Component {
  ...
  addreview = async () => {
    if (this.state.overall_rating.length > 1) {
      this.setState({overall_rating_check: 'Only one number can be entered'});
    } else if (this.state.overall_rating == '') {
      this.setState({
        overall_rating_length_check: "overall rating can't be empty",
      });
    }
    if (this.state.price_rating.length > 1) {
      this.setState({price_rating_check: 'Only one number can be entered'});
    } else if (this.state.price_rating == '') {
      this.setState({price_rating_length_check: "price rating can't be empty"})
    }
    if (this.state.quality_rating.length > 1) {
      this.setState({quality_rating_check: 'Only one number can be entered'});
    } else if (this.state.quality_rating == '') {
      this.setState({
        quality_rating_length_check: "quality rating can't be empty",
      });
    }
    if (this.state.clenliness_rating.length > 1) {
      this.setState({
        clenliness_rating_check: 'Only one number can be entered',
    
    })} else if (this.state.clenliness_rating == '') {
      this.setState({
        clenliness_rating_length_check: "clenliness rating can't be empty",
    })}
    ...
    return (
      <ScrollView>
        ...
        <Text style={{color: 'red'}}>
          {this.state.overall_rating_length_check}
        </Text>
        ...
        <Text style={{color: 'red'}}>
          {this.state.price_rating_length_check}
        </Text>
        ...
        <Text style={{color: 'red'}}>
          {this.state.quality_rating_length_check}
        </Text>
